...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateNavbar(user) {
        ...
        if (user) {
            ...
            // Get user initials or names from metadata
            const firstName = user.user_metadata?.first_name || 'U'
            const lastName = user.user_metadata?.last_name || ''

            // Option A: Link it to a profile page
            /* profile.onclick = () => window.location.href = 'profile.html' */

            //Get popup


            // Option B: Inject initials or an avatar image
            profileContent.innerHTML = `
      <span class="text-sm font-bold uppercase">${firstName[0]}${lastName[0] || ''}</span>
    `
            // If you have an avatar URL in metadata:
    ...
    async function getUserData() {
        ...
        if (data) {
            ...
            // Attach event listener immediately after HTML injection
